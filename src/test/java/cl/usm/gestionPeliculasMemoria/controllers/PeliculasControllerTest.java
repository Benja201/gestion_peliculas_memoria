...
class PeliculasControllerTest {
    ...
    void getAll_ok() {
        ...
        when(peliculasService.getAll()).thenReturn(Arrays.asList(p1, p2));
    ...
    void testGetAll() {
        Pelicula p1 = new Pelicula("1", "Cars", "Nolan", null, null);
        when(peliculasService.filter("Cars")).thenReturn(List.of(p1));
        ResponseEntity<List<Pelicula>> response = peliculasController.getAll("Cars");
        assertEquals(200, response.getStatusCode().value());
        assertNotNull(response.getBody());
    ...
    void createPelicula_ok() {
        Pelicula pelicula = new Pelicula("1", "Cars", "Nolan", "ABC123", null);
    ...
    void getComentarios() {
        ...
        assertEquals(200, response.getStatusCode().value());
        Comentario[] resultado = (Comentario[]) response.getBody();
